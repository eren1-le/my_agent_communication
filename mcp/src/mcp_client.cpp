 ...
 * @Date: 2026-03-24 09:06:51
 * @LastEditors: eren dengdengd1222@mail.com
 * @LastEditTime: 2026-03-24 22:09:07
 * @FilePath: /my_agent_communication/mcp/src/mcp_client.cpp
 * @Description: 
 * 
 */
...
MCPResponse MCPClient::callTool(const std::string& tool_name, const std::string& arguments) {
    ...
    if (reader.parse(arguments, args_json)) {
        params["arguemnts"] = args_json;
    } else {
        params["arguments"] = Json::Value(Json::objectValue);
    }

    // 只设置params 部分, 不包含method和id
...
MCPResponse MCPClient::receiveResponse() {
    ...
    //等待响应
    if (queue_cv_.wait_for(locker, std::chrono::seconds(30), [this] { return !response_queue_.empty(); })) {
...
MCPResponse MCPClient::receiveResponseSSE() {
    ...
    if (queue_cv_.wait_for(locker, std::chrono::milliseconds(timeout_ms),
