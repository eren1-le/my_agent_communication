...
public:
    ...
    std::vector<AgentRegistration> find_agents_by_tag(const std::string& tag) {
        ...
        for (const auto& agent_id : tag_it->second) {
            auto agent_item = agents_.find(agent_id);
            if (agent_item != agents_.end()) {
                result.push_back(agent_item->second);
            }
        }
        return result;
    }
    ...
    void check_health() {
        ...
        std::vector<std::string> to_remove_id;
        ...
        for (const auto& pair : agents_) {
            auto elapsed = std::chrono::duration_cast<std::chrono::seconds>(
                ...
                if (elapsed >= heartbeat_timeout_) {
                    to_remove_id.push_back(pair.first);
                }
        }
        // 移除超时的Agent
        for (const auto& agent_id : to_remove_id) {
            auto it = agents_.find(agent_id);
