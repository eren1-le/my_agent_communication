...
public:
    ...
    //获取统计信息
    CircuitBreakerStates getStats() const { return stats_; };
...
public:
    CircuitBreakerDecorator(const std::string& service_name,
                ...
                if (!circuit_breaker_) {
                    circuit_breaker_ = std::make_shared<CircuitBreaker>(config);
                    CircuitBreakerManager::getInstance().getCircuitBreaker(service_name) = circuit_breaker_;
                }
