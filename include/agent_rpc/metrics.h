 ...
 * @Date: 2026-03-16 20:37:42
 * @LastEditors: eren dengdengd1222@mail.com
 * @LastEditTime: 2026-03-20 10:55:04
 * @FilePath: /my_agent_communication/include/agent_rpc/metrics.h
 * @Description: 
 * 
 */
#pragma once
...
struct HistogramBucket {
    HistogramBucket(double upper_bound_ = 0) : upper_bound(upper_bound_) {
    ...
    double upper_bound;
    mutable std::atomic<uint16_t> count{0};
};
...
public:
    ...
    const std::vector<HistogramBucket>& getBuckets();
    void reset();
...
public:
    ...
    // RPC相关指标
    void recordRpcRequest(const std::string& service, const std::string& method, double durations_ms);
