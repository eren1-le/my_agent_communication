...
class LogAppender {
public:
    virtual ~LogAppender() = default; // 添加虚函数是为了防止多态的时候派生类不调用析构函数，导致内存泄漏
...
public:
    FileAppender(const std::string&filename,
                 size_t max_file_size = 100 * 100 * 1024,
                 int max_files = 10);
    ~FileAppender();
...
private:
    ...
    mutable std::mutex appenders_mutex_; // mutable 使变量在常函数中可以修改
};
//主日志器
class Logger {
public:
...
//便利宏定义，FILE捕获当前文件，LINE捕获当前行数FUNCTION捕获当前函数
