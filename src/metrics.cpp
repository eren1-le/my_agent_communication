 ...
 * @Date: 2026-03-16 20:37:53
 * @LastEditors: eren dengdengd1222@mail.com
 * @LastEditTime: 2026-03-20 10:45:04
 * @FilePath: /my_agent_communication/src/metrics.cpp
 * @Description: 
 * 
 */
...
HistogramMetric::HistogramMetric(const std::string& name, 
    ...
    for (double bucket : buckets) {
        HistogramBucket b(bucket);
        buckets_.emplace_back(b);
    }
}
std::string HistogramMetric::toString() const {
    ...
    // 输出桶计数
    std::lock_guard<std::mutex> locker(buckets_mutex_);
...
const std::vector<HistogramBucket>& HistogramMetric::getBuckets() {
    std::lock_guard<std::mutex> locker(buckets_mutex_);
    return buckets_;
}
...
void SummaryMetric::observe(double value) {
    ...
    std::lock_guard<std::mutex> locker(values_mutex_);
...
void SummaryMetric::updateQuantiles() {
    ...
    std::vector<double> sorted_values_ = values_;
    ...
    for (double quantile : quantiles_) {
        ...
        size_t index = static_cast<size_t>(quantile * (sorted_values_.size() - 1));
        if (index >= sorted_values_.size()) {
            index = sorted_values_.size() - 1;
        }
        quantile_values_[quantile] = sorted_values_[index];
    }
}
...
std::shared_ptr<CounterMetric> MetricsCollector::createCounter(const std::string& name,
    ...
    if (it != metrics_.end()) {
        return std::static_pointer_cast<CounterMetric>(it->second); //static_pointer用于将基类指针换回派生类
    }
...
void Metrics::recordRpcRequest(const std::string& service, const std::string& method, double duration_ms) {
    ...
    if (!rpc_duration_histogram_) {
        rpc_duration_histogram_ = MetricsCollector::getInstance().createHistogram(
            "rpc_duration_ms", "RPC request duration in millisecond",
