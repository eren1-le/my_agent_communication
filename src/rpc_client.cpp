...
bool RpcClient::sendMessage(const agent_communication::Message& message,
                            ...
                            int timeout_seconds) {
    if (!connected_) {
        LOG_INFO("Client not connected to server");
        return false;
    }
    ...
    try {
        ...
        if (status.ok()) {
            ...
            metrics.recordRpcResponse("AgetnCommunicationService", "SendMessage", response.status().code());

            if (response.status().code() == 0) {
                LOG_INFO("Message sent succussfully: " + response.message_id());
                return true;
            } else {
...
void RpcClient::realTimeCommuncationLoop() {
    try {
        grpc::ClientContext context;
...
void RpcClient::setupChannel() {
    ...
    args.SetMaxSendMessageSize(config_.max_massage_size);
