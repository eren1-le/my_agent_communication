 ...
 * @Date: 2026-03-18 20:51:26
 * @LastEditors: eren dengdengd1222@mail.com
 * @LastEditTime: 2026-03-20 14:23:14
 * @FilePath: /my_agent_communication/src/rpc_server.cpp
 * @Description
 * 
 */
...
void AgentCommunicationServiceImpl::registerAgent(const std::string& agent_id, const ServiceEndpoint& endpoint) {
    ...
    agent_message_queues_.emplace(agent_id, MessageQueue<agent_communication::Message>());
...
void AgentCommunicationServiceImpl::unregisterAgent(const std::string& agent_id) {
    std::lock_guard<std::mutex> locker(agents_mutex_);
...
void AgentCommunicationServiceImpl::cleanupOfflineAgents() {
    ...
    auto timeout = std::chrono::seconds(60); // 60s超时
...
grpc::Status AgentCommunicationServiceImpl::GetAgents(grpc::ServerContext* context,
    ...
    try {
        ...
        auto& metrics = Metrics::getInstance();
        metrics.recordRpcError("AgenCommunicationService", "GetAgents", "InternalError");
...
grpc::Status AgentCommunicationServiceImpl::BatchSendMessages(grpc::ServerContext* context,
    ...
    try {
        ...
        metrics.recordRpcRequest("AgentCommunicationService", "BatchSendMessage", 0);
...
void RpcServer::setupServer() {
    ...
    builder.SetMaxSendMessageSize(config_.max_massage_size);
...
void RpcServer::setupSslCredentials() {
    ...
    if (config_.enable_ssl && !config_.ssl_cert_path.empty() && !config_.ssl_key_path.empty()) {
        ...
        pkcp.private_key = readFile(config_.ssl_key_path);   // 读取私钥文件
        pkcp.cert_chain = readFile(config_.ssl_cert_path);   // 读取证书文件
